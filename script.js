...
document
  .querySelectorAll('a[href^="#"]:not(.sidebar-link)')
  .forEach((anchor) => {
...
const openContactFormBtns = document.querySelectorAll(
  ".js-contact-form-trigger"
);
...
contactForm.addEventListener("submit", (e) => {
  ...
  const mailtoLink = `mailto:contact@tomjanvier.com?subject=${encodeURIComponent(
    getLocalizedText("contactEmailSubject") + subject
...
function renderResources() {
  ...
  if (
    focus2030Container &&
    RESOURCES.focus2030 &&
    RESOURCES.focus2030.length > 0
  ) {
    const html = `
            <h4 class="resources-title">${getLocalizedText(
              "resourcesTitle"
            )}</h4>
                    ...
                    <li class="resource-item">
                        <a href="${
                          resource.url
                            ...
                            <span class="resource-icon">${getResourceIcon(
                              resource.type
                            )}</span>
  ...
  // Coordination SUD
  const coordinationSudContainer = document.getElementById(
    "resources-coordinationsud"
  );
  if (
    coordinationSudContainer &&
    RESOURCES.coordinationSud &&
    RESOURCES.coordinationSud.length > 0
  ) {
    const html = `
            <h4 class="resources-title">${getLocalizedText(
              "resourcesTitle"
  ...
  // Mouvement Européen
  const mouvementEuropeenContainer = document.getElementById(
    "resources-mouvementeuropeen"
  );
  if (
    mouvementEuropeenContainer &&
    RESOURCES.mouvementEuropeen &&
    RESOURCES.mouvementEuropeen.length > 0
  ) {
    const html = `
            <h4 class="resources-title">${getLocalizedText(
              "resourcesTitle"
  ...
  if (
    amnestyContainer &&
    RESOURCES.amnestyInternational &&
    RESOURCES.amnestyInternational.length > 0
  ) {
    const html = `
            <h4 class="resources-title">${getLocalizedText(
              "resourcesTitle"
